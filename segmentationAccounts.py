...
from zcrmsdk.src.com.zoho.crm.api.bulk_write import BulkWriteOperations, FileBodyWrapper, UploadFileHeader, \
    SuccessResponse, APIException, RequestWrapper, Resource, FieldMapping, CallBack
...
def upload_file():
    ...
    stri = s3_resource.get_object(Bucket='ds-autosegment', Key='adv_seg_jobs_jmng/deployed/segment_output_jmng.csv')
    # stri = s3_resource.get_object(Bucket='awethu-test0', Key='segmentAccount - Sheet1 (3).csv')

    train_label_string = stri['Body'].read().decode('utf-8')

    read_train_labels = pd.read_csv(StringIO(train_label_string))

    read_train_labels.to_csv("segmentationAccounts.csv")
    with ZipFile("SEGMENTATIONACCOUNTS.zip", "w") as newzip:
        newzip.write("segmentationAccounts.csv")

    stream_wrapper = StreamWrapper(file_path="SEGMENTATIONACCOUNTS.zip")
...
def create_bulk_write_job(module_api_name):
    ...
    field_mapping = FieldMapping()
    field_mapping.set_api_name('CCSegment')
    field_mapping.set_find_by('id')
    field_mapping.set_index(3)
    field_mappings.append(field_mapping)

    field_mapping = FieldMapping()
    field_mapping.set_api_name('Updated')
    ...
    if response is not None:
        ...
        if response_object is not None:
            ...
            if isinstance(response_object, SuccessResponse):
                ...
                # Get the Message
                print("Message: " + response_object.get_message().get_value())
                os.remove("segmentationAccounts.csv")
                os.remove("SEGMENTATIONACCOUNTS.zip")
            ...
            # Check if the request returned an exception
                ...
                print('HttpStatus: ' + response_object.get_http_status())
create_bulk_write_job("Accounts")
# upload_file()